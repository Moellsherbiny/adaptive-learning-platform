...
export default function PhotoshopSimulationPage() {
  ...
  // حدود مكان الورقة البيضاء داخل صورة الفوتوشوب
const workArea = { x: 103, y: 154, w: 471, h: 387 };

const isInsideWorkArea = (x: number, y: number) => {
  return (
    x >= workArea.x &&
    x <= workArea.x + workArea.w &&
    y >= workArea.y &&
    y <= workArea.y + workArea.h
  );
};
...
const [section, setSection] = useState<'save' | 'draw' | 'toolbox' | 'final'>('save');
  ...
  const handleDebugClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
        ...
        {section === 'save' && (
          <section style={sectionStyle}>
            ...
            <div onClick={handleUiClick} style={{ position: 'relative', display: 'inline-block' }}>
              <img src={images[uiImage]} alt="واجهة فوتوشوب" style={{ width: '98%', maxWidth: 900, height: 'auto', border: '1px solid #ccc' }} />
              {/* Hidden hit areas */}
              {/* File */}
              {uiImage === 'step1' && (
                <button
